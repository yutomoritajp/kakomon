...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="指定範囲のPDFを一時フォルダに作成します。")
    parser.add_argument("--period", dest="period", required=True, choices=[p.value for p in Period])
    parser.add_argument("--section", dest="section", required=True, choices=[s.value for s in Section])
